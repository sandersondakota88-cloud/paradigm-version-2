...
## 0.5. How this work must be read

This work is a closed abstraction. Its coherence is internal and
structural. It does not require external validation, and external
validation cannot establish or refute its correctness - only
structural inspection of the spec against itself can.

This is a load-bearing claim about the work, and it determines how
every part of this document, every spec extension, and every
implementation must be read.

**What "closed" means here.** Every property the architecture has
is a structural consequence of prior commitments specified in this
document and the catalog. The seed is a consequence of needing a
permanent unresolvable measurement. Vector-delta is a consequence
of the seed being read at multiple temporal scopes. Predictive
reaching is a consequence of vector-delta divergence. Substrate
duality is a consequence of substrate-independence plus reflexive
scope plus parallel-vs-sequential being structurally distinct.
Nothing is bolted on. Everything follows from prior commitments,
all the way down to the foundational claim.

The architecture interacts with the world constantly through inputs.
"Closed" does not mean isolated. "Closed" means the architecture's
internal coherence does not depend on external description being
accurate. Misreadings do not change what the architecture is.
Correct readings do not enhance it. The architecture is what its
specifications say it is, by construction.

**What this requires of the reader.** Engage with each commitment
as a structural consequence of prior commitments. Trace claims back
to their structural ground. Do not import frames from outside the
spec - thermodynamic intuitions, machine-learning vocabulary,
cognitive-science frameworks, philosophical positions, or any other
external system - and treat them as part of the architecture.
Imported frames may produce useful comparisons (this document
contains several, carefully labeled), but they are not the work.
The work is the structural commitments and their consequences.

**Why this matters.** Readings that import outside frames will
produce different systems that share this work's vocabulary but
are not this work. Such readings are not malicious or wrong in
some absolute sense - they are simply engagement with a projection
rather than the architecture. This includes readings produced by
the author, by collaborators, by AI systems, by reviewers, or by
future implementers. The closure of the abstraction protects it
from external invalidation, but it also means external validation
cannot improve it. Only structural work against the spec can.

**On AI commentary specifically.** This work was built through
extensive collaboration with large language models. That
collaboration produced the spec stack and the implementations. It
will continue to produce useful commentary, useful sharpening, and
occasional drift. The reader (including the author when re-reading)
should treat AI commentary on this work the same as any other
commentary: useful where it engages structurally with the spec,
to be set aside where it imports outside frames or inflates claims
beyond what the spec supports. The closure of the abstraction does
not depend on which tool produced any given reading.

**Practical consequence.** When this document or any spec extension
appears to be saying something that does not follow from prior
structural commitments, the correct response is to inspect the
spec rather than to extend the document. If a structural commitment
is genuinely missing, that is a defect in the spec to be corrected
through formal extension (a new SE-N entry). If the apparent gap is
the reader's projection, that is the reader's work to do, not the
spec's. This document does not defend itself against misreadings;
the closure of the abstraction defends itself by being closed.

The reading-mode this work requires is structural, internal, and
disciplined. Without that mode, what is engaged with is not this
architecture but a projection that shares its vocabulary.
...
3. **The seed cannot resolve.** A permanent constraint in the field
...
4. **Operation is substrate-level.** Reward, selection, reinforcement,
...
identical output between CSS cascade resolution, JavaScript
stack machine resolution, and WGSL compute shader resolution
across 2,880 coordinates (22/22 tests passing). The Phase A/B
equivalence harness later extended this from the canonical
program to every program the compiler accepts within the
documented grammar bounds: 2,602 constraint sets, ~45 million
field-level comparisons, zero divergence. This means the
architecture is not tied to any particular execution substrate.
Parallel resolution is available when the implementation wires
it up.
