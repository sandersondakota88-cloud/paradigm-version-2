...
1. Heuristic discrimination on a single axis (which is what frequency
   counters, statistical tokenizers, and most ML approaches do; they pick
   one axis, project onto it, and accept the noise).
1. Hand-written rules that encode multidimensional knowledge as
...
1. **Concurrent state intersection** is the substrate’s execution model:
...
1. The **substrate stack** is a parallel topology of autonomous
   ...
   number of axes.
1. **Structure** is defined relative to the substrate’s chosen axis set.
...
1. **The migration argument** rests on multidimensional discrimination
...
1. **The wide claim** rests on the dimensional pattern generalizing
