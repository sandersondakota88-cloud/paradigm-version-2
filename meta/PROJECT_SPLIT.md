...
SE-01 commits to operationally manifest, and exercises the split
at richer structural positions. Four sub-phases, ordered so
visibility ships first because subsequent sub-phases produce
structural events that need a surface to be observable:

- 4a. Reflexive surface: a parallel record of structural transitions
  the architecture undergoes, accumulating clauses describing what
  the field is currently doing structurally, sourced from
  field-state transitions in vocabulary drawn from the field
  itself. Read-only with respect to the field (per O1), bounded
  buffer (per O2), vocabulary from the field (per O3).
- 4b. Cross-substrate compound constraints: a new constraint kind
  ...
  Compound formation events fire reflexive surface clauses.
- 4c. Storage as substrate: ER engine evaluates over a combined
  population of live and persisted constraints. Recall becomes
  parallel matching against past structure, byte-equivalent
  between CPU oracle and GPU path. Recall events fire reflexive
  surface clauses.
- 4d. Reflexive surface from promoted compounds (Pass B): the
  surface's templates source from promoted compound constraints
  rather than from a fixed list. The system's accumulated
  structure becomes the grammar of its own self-account.

Estimated scope: approximately 2750 lines combined across sub-
...
PROJECT_SPLIT.md v1.2, pinned to SE-06 v1.0, DEFINITION.md v1.1
(with section 0.5), INVARIANTS.md v1.2, and IMPLEMENTATION_PATH.md
v2.4.

v1.2 changes:

- Reordered Phase 4 sub-phases to match IMPLEMENTATION_PATH.md
  v2.1+: 4a reflexive surface, 4b cross-substrate compounds, 4c
  storage as substrate, 4d promoted-compound templates. The
  reordering puts visibility first because subsequent sub-phases
  produce structural events that need a surface to be observable.
...
- Added 4d: reflexive surface Pass B (templates from promoted
  compounds), formerly part of 4c output surface.
- Updated total Phase 4 scope estimate to ~2750 lines.
