...
**Implementations: Phase 5 shipped.**
...
- Phase A/B equivalence harness (shape-exhaustive coverage):
  2,602 generated constraint sets verified CSS == JS == GPU
  byte-identical, ~45 million field-level comparisons, zero
  divergence. Algorithm 16 status flipped PROPOSED -> IMPLEMENTED.
...
- Phase 1: bootstrap-fresh-v2 shipped
- Phase 2: ER engine + CPU oracle + WGSL shader: equivalence
  verified
- Phase 3: SE-06 implementation, 8/8
- Phase 4a-4d: expressive substrate, 53/53 across the four sub-
  phases
- Phase 5: coupling verification, 47 stress tests + 7 static
  coupling-path checks, 115/115 with full regression
- KindMult audit (Phase 5 deliverable): selection-bias constants
  identified as dead-code (no consumer reads the ranking).
  Recommendation deferred to future phase.

**What runs: SE-06 implementation through Phase 4d, verified
under stress per Phase 5.** The two-engine system (ER + CT
coupled through delta) operates with reflexive surface,
storage substrate, cross-substrate compound constraints,
and Pass B templates from promoted compounds, all under
the substrate-independence claim algorithm 16 establishes.
...
**Phase 5: coupling verification.** SHIPPED.
Built tests and diagnostic tooling that verify the engines
remain coherent under stress. Cross-engine ratification, divergence
induction, persistence-across-restart, compound coherence, rapid-
input-stream tests, substrate equivalence at scale, and a
kindMult audit. 47 stress tests plus 7 static coupling-path
checks; 115/115 with prior regression.
...
named in section 9’s “Let delta decide before imposing precedence”
...
## 7. Phase 5.5: imposed-precedence cleanup

### Purpose

Act on the kindMult audit finding from Phase 5. The audit
demonstrated that selection-side bias constants produce no
observable change in primary metrics because no downstream consumer
reads selectFromMatches' ranking. The architecture has been
computing a sorted ranking that nothing consumes; the constants
shaping the ranking are imposed precedence per section 9 principle
4, with no work to do.
...
weight composition. Downstream consumers (markUsed,
updateCorrelations, ratify) continue to operate on the set as they
already do.
...
explicit accumulator read by the surface display. This is a
separate K3-letter compliance question, named in INVARIANTS as a
known gap, not fixed in 5.5.
...
which the current kernel does not provide. Bumps KERNEL to v1.1.
...
Bumps INVARIANTS to v1.2 (which also includes the v1.1 O-class
addition that had been forward-referenced but not promoted).
...
- selectFromMatches: drop effectiveWeight composition, drop
  ...
  namedBias multiplication, drop kindMult ladder, drop sort.
  Return matched set with {idx, kind, named} metadata.
...
removal should be transparent. The risk runs the opposite
direction: if any test was implicitly depending on a constraint
kind appearing preferentially in chosen ordering (which the audit
says nothing does), removal would expose that. Test runs verify.
...
random draw with substrate-tension-derived weights, threshold
cutoff with substrate state determining the cutoff, or something
else. Per section 9 principle 4 (let delta decide before imposing
precedence), the weights that feed the rank should derive from
substrate state (fast/slow layer modulation) and accumulated weight
history rather than from imposed kind multipliers.
...
SE extension depends on what behavior under Phase 5 stress
conditions reveals about whether selection rank-sensitivity is
needed at the substrate's current scale.
...
## 10. What this path commits to and what it does not
...
## 11. What was the original ROADMAP and what changed
...
section 0.5), KERNEL.md v1.1 (section 5 rewritten for Phase 5.5),
INVARIANTS.md v1.2 (O-class promoted; K2 and K3 implementation
notes added), PROJECT_SPLIT.md v1.2 (Phase 4 sub-phases reordered
to match v2.1+), and SE-01 through SE-06. Supersedes the original
ROADMAP.md and prior versions of this document.

v2.4 changes:

- Added Phase 5.5 (imposed-precedence cleanup) between Phase 5
  and Phase 6. Acts on the Phase 5 kindMult audit finding by
  removing six imposed-precedence constants (ratified=1.3,
  meta=1.15, compound=1.25, namingBonus=1.5, recencyExp=1.5,
  _enforceCaps kindBonus=15) and the dead effectiveWeight
  ranking computation. Selection becomes set-computation
  honestly under the current spec.
- Renumbered: v2.3's section 7 (Phase 6) to section 8;
  section 8 (Order of work) to 9; section 9 (Commits / Does
  not commit) to 10; section 10 (What changed) to 11;
  section 11 (Reading) to 12. Updated forward references in
  Phase 4 risk text and Phase 5.5 scope text accordingly.
- Named Phase 5.6 (wire-a-consumer) inside the Phase 5.5
  section as a possible future extension that would realize K2
  part (a) and K3 by adding a rank-consuming selection
  mechanism. Not scheduled.
- Promoted INVARIANTS.md v1.0 to v1.2 in a single step,
  folding the v1.1 O-class additions (forward-referenced by
  v2.1+ but not previously promoted in-tree) plus the K2/K3
  implementation notes from this version. PROJECT_SPLIT.md
  v1.1 promoted to v1.2 to match the v2.1 Phase 4 reorder
  (similarly forward-referenced but not previously promoted).
  This drop reconciles the spec-stack pinning that prior
  versions carried as aspirational.

v2.3 changes:

- Phase 5 shipped. 47 new stress tests (5a-5f) plus 7 static
  coupling-path checks added. Full regression: 115/115 (54 new
  Phase 5 + 61 prior).
- KindMult audit completed (per v2.2 section 8 principle 4).
  Finding: under tested streams, the selection-bias constants
  (ratified=1.3, meta=1.15, compound=1.25, namingBonus=1.5,
  recencyExp=1.5) produce no observable change in primary
  metrics. Investigation revealed the selection ranking is
  computed and sort-keyed but never read by downstream
  consumers. The constants modulate sort order; sort order
  is not consumed. Recommendation (advisory): either remove
  the dead constants and effectiveWeight computation, or wire
  a consumer that reads the ranking. Either change is out of
  scope for Phase 5 (verification only) and would belong to
  a future Phase 5.5 or 6, with the audit re-run after to
  verify the new pipeline.
- Phase 5’s S2 verification: CPU oracle is deterministic and
  byte-equivalent to the ER engine’s CPU path. GPU-side
  byte-equivalence inherits from Phase 2’s test-equivalence.js
  (22/22 across CSS, JS, WGSL substrates).
- Coupling-path audit (7/7): SE-06’s no-command-path
  commitment (S3) verified at the source level. CT binds to
  ER (allowed); ER does not bind to CT; surface and storage
  do not bind to engines.
