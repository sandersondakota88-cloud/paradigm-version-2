...
function install(FieldModule, opts) {
  ...
  // ---- wrap Field.serialize / Field.deserialize to preserve intake ----
  // Per SE-08: intake records ARE field state (the field's input history
  // within the bounded retention window). Persistence captures the field;
  // capturing must include intake. Phase 9 F2 closure on this point.
  const originalSerialize = Field.serialize.bind(Field);
  Field.serialize = function () {
    const baseJson = originalSerialize();
    let data;
    try { data = JSON.parse(baseJson); }
    catch (_) { return baseJson; }
    data.intake = {
      records: Field.intake ? Field.intake.records : [],
      totalReceived: Field.intake ? Field.intake.totalReceived : 0,
      cap: Field.intake ? Field.intake.cap : DEFAULT_CAP
    };
    return JSON.stringify(data);
  };

  const originalDeserialize = Field.deserialize.bind(Field);
  Field.deserialize = function (json) {
    const ok = originalDeserialize(json);
    // Even if the base deserialize returned false (version mismatch),
    // intake recovery still attempts. Return ok per the base contract.
    let data;
    try { data = JSON.parse(json); } catch (_) { return ok; }
    if (Field.intake) {
      if (data && data.intake) {
        Field.intake.records = Array.isArray(data.intake.records)
          ? data.intake.records.slice() : [];
        Field.intake.totalReceived = +data.intake.totalReceived || 0;
        if (typeof data.intake.cap === "number" && data.intake.cap > 0) {
          Field.intake.cap = data.intake.cap;
        }
      } else {
        // Pre-F2 snapshot without intake: clear instead of leaving stale
        Field.intake.clear();
        Field.intake.totalReceived = 0;
      }
    }
