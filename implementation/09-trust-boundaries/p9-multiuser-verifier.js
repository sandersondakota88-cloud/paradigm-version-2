...
//
// Contact records and updates flow through intake (SE-08), per F2 Route A.
// Per F2: domain records are NOT kind="data" constraints; the field's
// specified constraint kinds are seed/derived/predictive/ratified/meta.
// Domain records are intake-pathway events; the field's intake extension
// persists them as part of substrate state (Phase 9 F2 closure).
// "Current view" reduces over the intake history.
// F5 honored: every update is a separate intake record; no overwrites.
// ----------------------------------------------------------------------------

function createContact(field, contact, ts, source) {
  field.intake.publish({
    type: "domain::contact-record",
    value: Object.assign({ _ts: ts || 0, _from: source || "local" }, contact),
    timestamp: ts || 0,
    source: source || "local"
  });
}
...
function currentContactView(field, contactId) {
  ...
  const updates = [];
  for (const r of field.intake.records) {
    if (r && r.type === "domain::contact-update" &&
  ...
  updates.sort(function (a, b) {
    ...
    // Tie-break on source for determinism
...
function updateCountForContact(field, contactId) {
  ...
  for (const r of field.intake.records) {
    if (r && r.type === "domain::contact-update" &&
        r.value && r.value.contactId === contactId) n++;
  }
  return n;
}
...
async function main() {
  ...
  await asyncTest("Each side commits its own snapshot; addresses differ", async () => {
    ...
    assert(artifactA);
    assert(artifactB);
    // The number of update intake records on each side should match.
    // Per F2: domain records flow through intake; field-intake-extension
    // wraps Field.serialize so intake records are part of fieldData.
    const aUpdates = artifactA.fieldData.intake.records.filter(
      r => r && r.type === "domain::contact-update").length;
    const bUpdates = artifactB.fieldData.intake.records.filter(
      r => r && r.type === "domain::contact-update").length;
    assert(aUpdates === 1, "expected 1 update on A; got " + aUpdates);
  ...
  await asyncTest("Reports (P6) on each side see the integrated history", async () => {
    const A = buildInstance({ id: "A" });
    const B = buildInstance({ id: "B" });
    // Add deals to BOTH sides separately. Per F2 (Phase 9 plan):
    // domain records flow through intake (SE-08), not as kind="data"
    // constraints. P6's report templates fold over intake records.
    A.field.intake.publish({
      type: "domain::deal-record",
      value: { id: "1", stage: "discovery", owner: "alice" },
      timestamp: 0,
      source: "p9-fixture"
    });
    B.field.intake.publish({
      type: "domain::deal-record",
      value: { id: "2", stage: "qualified", owner: "bob" },
      timestamp: 0,
      source: "p9-fixture"
    });
    ...
    assert(aReport.total === 1, "A total expected 1; got " + aReport.total);
