  ...
  for (const rule of rules) {
    ...
    if (sel && sel.kind === "compound") {
      for (const attr of sel.attributes) {
        // CSS admits presence-only attribute selectors (e.g.
        // [data-x] without a value). Per Phase 9 architectural
        // decision (PHASE_9_PLAN sec 15) and LESSONS sec 10
        // (grammar-coverage discipline): the architecture should
        // admit what the grammar admits. Presence-only is supported
        // by synthesizeOne (sets selectorMap[name] = "*"); the
        // subset validator now admits them too. This makes catch-
        // all rules expressible (e.g. an outer cascade that emits
        // a default coord value when a more specific rule does not
        // match).
