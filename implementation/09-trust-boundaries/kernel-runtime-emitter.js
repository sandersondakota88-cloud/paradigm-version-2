...
(function () {
  ...
  // Per Phase 9 architectural decision (PHASE_9_PLAN sec 15) and audit
  // P3: the deposition's bridge binding is embedded inline as
  // __DEPOSITION_BRIDGE_BINDING__. When present, it's passed to
  // bridge.init() and used for the trigger query. When absent,
  // bridge defaults preserve today's behavior.
  var binding = globalThis.__DEPOSITION_BRIDGE_BINDING__ || null;
  var bridge = null;
  ...
  try {
    if (DOMbridge && typeof document !== "undefined" &&
      ...
      if (stateElement) {
        ...
        bridge.init(Field, stateElement, binding);
        // Auto-wire click handlers per contract sec 4 default. The
        // trigger attribute name comes from the binding when present;
        // defaults to "data-trigger".
        if (typeof document.querySelectorAll === "function") {
          var triggerAttr = (binding && binding.defaultEventTriggerAttr)
            || "data-trigger";
          var triggers = document.querySelectorAll("[" + triggerAttr + "]");
...
function buildHtml(opts) {
  ...
  //
  // Per Phase 9 architectural decision: the presence marker attribute
  // comes from the deposition's binding (when provided), allowing the
  // deposition to ship its own coord conventions. Defaults to
  // "data-substrate-state" preserving today's behavior.
  const presenceMarker = (opts.binding && opts.binding.presenceMarkerAttr)
    || "data-substrate-state";
  lines.push("<div id=\"substrate-state\" " + presenceMarker + "=\"\" " +
  ...
  // Bridge binding (deposition-specific conventions). When provided,
  // SKELETON_INIT passes this to bridge.init() so the bridge uses
  // deposition-specific attribute names instead of today's defaults.
  // Per Phase 9 architectural decision: configuration moves from
  // bridge code into deposition.
  if (opts.binding) {
    lines.push("<script>");
    lines.push("/* Bridge binding (deposition conventions) */");
    lines.push("globalThis.__DEPOSITION_BRIDGE_BINDING__ = " +
      JSON.stringify(opts.binding) + ";");
    lines.push("</script>");
  }
