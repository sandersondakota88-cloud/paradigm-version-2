...
// ============================================================================
//
// Phase 9 F2: domain records flow as intake records, not as kind="data"
// constraints. The latest intake record per record id determines that
// record's current state (event-sourced model, consistent with P9's
// contact-update pattern).
//
// Conventions:
//   - Deals arrive as intake records with type="domain::deal-record"
//     and value = { id, stage, owner, ... }
//   - Contacts arrive as intake records with type="domain::contact-record"
//     and value = { id, status, ... }
//   - Each subsequent record for the same id supersedes earlier records
//     (latest-wins per intake order)
//
// Each template:
//   - Takes a fieldSnapshot
//   - Returns plain output data
//   - Sources all output content from the snapshot (O3)
//   - Does not modify the snapshot (O1)
// ============================================================================

// Helper: scan intake for records of a given domain type, fold by id,
// return {id -> latest value} map. Latest-wins per intake order.
function _foldDomainRecords(intake, recordType) {
  const latest = {};
  if (!Array.isArray(intake)) return latest;
  for (const r of intake) {
    if (!r || r.type !== recordType) continue;
...
const TEMPLATES = Object.freeze({
  ...
  dealsByStage(snapshot) {
    const deals = _foldDomainRecords(snapshot.intake, "domain::deal-record");
    const counts = {};
    let total = 0;
    for (const id of Object.keys(deals)) {
  ...
  contactsByStatus(snapshot) {
    const contacts = _foldDomainRecords(snapshot.intake, "domain::contact-record");
    const counts = {};
    let total = 0;
    for (const id of Object.keys(contacts)) {
      const status = contacts[id].status;
