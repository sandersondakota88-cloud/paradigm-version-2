...
async function testNoKindMultForRecalled() {
  ...
  // Confirm that other kinds DO have multipliers (sanity check)
  const hasRatifiedMult = /c\.kind === ["']ratified["']/.test(selectBody);
  console.log("  selectFromMatches has 'ratified' kindMult (sanity): "
    + hasRatifiedMult);

  return {
    pass: hasRecalledMult === false && hasRatifiedMult === true
  };
}
