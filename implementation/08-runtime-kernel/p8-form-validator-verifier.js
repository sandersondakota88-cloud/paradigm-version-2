...
function simulateCascade(rules, coords) {
  for (const rule of rules) {
    ...
    if (allMatch) {
      return { matched: true, op: rule.emit.value, ruleId: rule.id, prop: rule.emit.property };
    }
  }
  return { matched: false };
}

// Walk all rules and apply ALL that match (cascade can produce multiple
// outputs from different rules)
function simulateCascadeAll(rules, coords) {
  ...
  for (const rule of rules) {
    ...
    if (allMatch) {
      outputs[rule.emit.property] = rule.emit.value;
    }
  }
  return outputs;
}
