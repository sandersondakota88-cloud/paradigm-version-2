...
// Helper: simulate the cascade evaluator's match against a coord set.
// Mirrors the logic in kernel-cascade-evaluator: a rule's pattern.selector
// is a {attrName: value} map; rule fires when ALL its selector entries
// match the current coords.
function simulateCascade(rules, coords) {
  for (const rule of rules) {
    const sel = rule.pattern.selector;
    let allMatch = true;
    for (const k of Object.keys(sel)) {
      // "*" wildcard means presence-only check
      if (sel[k] === "*") {
    ...
    if (allMatch) {
      return { matched: true, op: rule.emit.value, ruleId: rule.id };
    }
  }
