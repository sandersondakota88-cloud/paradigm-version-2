// phase-3-trajectory-vlan-sync.js
// =============================================================================
// Phase 11 reconciliation step: re-run the Phase 3.4 trajectory analysis
// against the original Phase 0 corpus (exodus-vlan-sync.html) instead of
// kernel/field.js.
//
// Per PLAN.md §9.3: the corpus shift to kernel/field.js compromised the
// SE-11 §2.5 falsification target ("substrate surfaces patterns a reader
// would recognize in an unfamiliar SPA"). Running on vlan-sync tests that
// claim against the originally-scoped unfamiliar corpus.
//
// JS-only scope per adapter-spec.md §1: extract the <script>...</script>
// block from the HTML and feed it to the existing JS corpus-adapter.
// =============================================================================
...
const VLAN_SYNC_PATH = path.join(PROJ_ROOT,
  "exodus", "demonstration", "state-projector", "exodus-vlan-sync.html");

const args = process.argv.slice(2);
const WINDOW_SIZE = parseInt(args[0] || "100", 10);
const OUT_PATH    = args[1] ||
  path.join(HERE, "phase-3-trajectory-vlan-sync.tsv");

// ---- Extract JS from vlan-sync HTML ----
const html = fs.readFileSync(VLAN_SYNC_PATH, "utf8");
const scriptStart = html.indexOf("<script>");
const scriptEnd   = html.indexOf("</script>");
if (scriptStart < 0 || scriptEnd < 0) {
  console.error("FATAL: no <script>...</script> block found in vlan-sync");
  process.exit(2);
}
const corpusSrc = html.substring(scriptStart + "<script>".length, scriptEnd);

console.log("==============================================================");
console.log("Phase 3.4 trajectory — vlan-sync corpus (reconciliation run)");
console.log("==============================================================");
console.log("Corpus:        " + path.relative(process.cwd(), VLAN_SYNC_PATH));
console.log("JS extracted:  " + corpusSrc.length + " bytes, " +
  corpusSrc.split("\n").length + " lines");
...
for (const peer of peers) {
  for (const m of METRICS) headerCols.push(peer + "." + m);
}
