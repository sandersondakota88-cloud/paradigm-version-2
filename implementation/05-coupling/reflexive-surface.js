...
const CLAUSE_KIND = Object.freeze({
  ...
  // Phase 4d: Pass B - compound-active uses templates derived from
  // promoted compounds. The surface emits when a promoted compound's
  // predicates currently both hold (render half against live constraint
  // state, exec half against current CT state).
  COMPOUND_ACTIVE: "compound-active"
});

// Clause source enumeration. Distinguishes Pass A (templates we wrote)
// from Pass B (templates derived from promoted compounds). A clause's
// source field marks which pool produced its form.
const CLAUSE_SOURCE = Object.freeze({
  PASS_A:  "pass-a",     // template authored in this module
  PASS_B:  "pass-b"      // template derived from a promoted compound
});

// ---------------------------------------------------------------------------
// Pass B: templates derived from promoted compounds
//
// In Pass A (Phase 4a), the surface emits clauses using templates
// written in this module. The system's vocabulary (token names, family
// types, sub-cascade names, op kinds) fills slots in our forms.
//
// Pass B (this phase) shifts which forms are available based on what
// compounds the field has promoted. A promoted compound's pattern is
// (renderPredicate, execPredicate). The pair (render-type, exec-type)
// selects a template form from PASS_B_TEMPLATES below. The compound's
// promotion gates the template's availability: without the compound,
// the template never fires.
//
// What this is, structurally:
//   - The form-set is finite and authored in this module
//   - Which forms become available is gated by compound promotion
//   - Which form fires for a given event is determined by the structural
//     shape of the compound, not by selection logic we encode
//   - Slot values come from current field state
//
// What this is not:
//   - The system does not invent forms beyond this set
//   - Pass B does not "describe itself" in any agentic sense; it selects
//     among templates whose availability the field's accumulation gates
//
// The honest reading: as the field accumulates promoted compounds, the
// proportion of surface clauses sourced from Pass B grows. The
// proportion is itself observable.
// ---------------------------------------------------------------------------

const PASS_B_TEMPLATES = Object.freeze({
  // (render-type, exec-type) -> template form
  // {render-slot} = a description of the render predicate
  // {exec-slot} = a description of the exec predicate state

  // has-token combinations
  "has-token::queue-depth":      "{token} surfaces with queue at {depth}",
  "has-token::recent-op":        "{token} surfaces after {opKind}",
  "has-token::exec-gap-state":   "{token} surfaces in {gapState} regime",

  // length-range combinations
  "length-range::queue-depth":   "length {lenRange} with queue at {depth}",
  "length-range::recent-op":     "length {lenRange} after {opKind}",
  "length-range::exec-gap-state": "length {lenRange} in {gapState} regime",

  // char-class combinations
  "char-class::queue-depth":     "{cls} class with queue at {depth}",
  "char-class::recent-op":       "{cls} class after {opKind}",
  "char-class::exec-gap-state":  "{cls} class in {gapState} regime",

  // co-occurs combinations
  "co-occurs::queue-depth":      "{a}+{b} with queue at {depth}",
  "co-occurs::recent-op":        "{a}+{b} after {opKind}",
  "co-occurs::exec-gap-state":   "{a}+{b} in {gapState} regime"
});

// Resolve a template key from a compound's pattern shape
function templateKeyForCompound(compound) {
  if (!compound || !compound.pattern) return null;
  if (compound.pattern.type !== "compound") return null;
  const r = compound.pattern.render;
  const e = compound.pattern.exec;
  if (!r || !e || !r.type || !e.type) return null;
  return r.type + "::" + e.type;
}

// Instantiate a Pass B template against current field state.
// Returns the rendered text or null if any slot cannot be filled.
function renderPassBTemplate(compound, field) {
  const key = templateKeyForCompound(compound);
  if (!key) return null;
  const tmpl = PASS_B_TEMPLATES[key];
  if (!tmpl) return null;
  ...
  // Render-side slot fills
  if (r.type === "has-token") {
    text = text.replace("{token}", "'" + r.token + "'");
  } else if (r.type === "length-range") {
    text = text.replace("{lenRange}", r.min + "-" + r.max);
  } else if (r.type === "char-class") {
    text = text.replace("{cls}", r.cls);
  } else if (r.type === "co-occurs") {
    text = text.replace("{a}", "'" + r.a + "'").replace("{b}", "'" + r.b + "'");
  }

  // Exec-side slot fills - read current state
  if (e.type === "queue-depth") {
    const d = (field && field.ctPendingOps) ? field.ctPendingOps.length : 0;
    text = text.replace("{depth}", String(d));
  } else if (e.type === "recent-op") {
    text = text.replace("{opKind}", e.kind);
  } else if (e.type === "exec-gap-state") {
    text = text.replace("{gapState}", e.state);
  }

  // Verify no unfilled slots remain (would indicate pattern shape mismatch)
...
class ReflexiveSurface {
  ...
  observe() {
    ...
    // Phase 4d: Pass B compound-active events. Templates derived from
    // promoted compounds become available as the field accumulates
    // them. The detector emits clauses for compounds that matched the
    // most recent input and have promoted (per Phase 4b promotion).
    const compoundActiveClauses = this._detectCompoundActive(prev, cur);
    ...
    const all = formedClauses
      ...
      .concat(compoundActiveClauses)
      .concat(settledClauses)
      .concat(divergedClauses);

    // Emit each
    for (const clause of all) {
      const stamped = this._emit(clause);
      emittedThisObserve.push(stamped);
    }
  ...
  // COMPOUND-ACTIVE (Phase 4d, Pass B): a promoted compound's
  // predicates currently both hold. Pass B template forms are derived
  // from the compound's structural shape (render-type x exec-type).
  // The form-set is finite and authored in this module; what gates
  // template availability is compound promotion, which is a property
  // of accumulated field structure.
  //
  // Detection: read Field.lastMatchedCompoundIds, filter to those that
  // are promoted, render the Pass B template for each. Emit one clause
  // per promoted-and-matched compound per step.
  //
  // Honest limit: we wrote the template forms. The compound's promotion
  // gates which forms become available, not which forms exist. Phase 4d
  // shifts which templates can fire (gated by accumulated structure),
  // not which templates are possible.
  _detectCompoundActive(prev, cur) {
    const out = [];
    if (!Field || !Field.constraints) return out;
    const matchedIds = Field.lastMatchedCompoundIds || [];
    if (matchedIds.length === 0) return out;
    // De-dup against this step's prior emissions (we only emit once
    // per compound per step). Tracked by matching against the most
    // recent clauses for compound-active at this step.
    const alreadyEmittedIds = Object.create(null);
    for (let i = this.clauses.length - 1; i >= 0; i -= 1) {
      const c = this.clauses[i];
      if (c.step !== Field.step) break;
      if (c.kind !== CLAUSE_KIND.COMPOUND_ACTIVE) continue;
      if (c.evidence && c.evidence.compoundId) {
        alreadyEmittedIds[c.evidence.compoundId] = true;
      }
    }

    for (const id of matchedIds) {
      if (alreadyEmittedIds[id]) continue;
      // Find the compound by id
      let compound = null;
      for (const c of Field.constraints) {
        if (c.id === id && c.kind === "compound") { compound = c; break; }
      }
      if (!compound) continue;
      if (!compound.promoted) continue;  // Pass B requires promotion

      const text = renderPassBTemplate(compound, Field);
      if (!text) continue;
      out.push({
        kind: CLAUSE_KIND.COMPOUND_ACTIVE,
        source: CLAUSE_SOURCE.PASS_B,
        text: text,
        evidence: {
          compoundId: compound.id,
          templateKey: templateKeyForCompound(compound),
          renderType: compound.pattern.render.type,
          execType: compound.pattern.exec.type,
          fidelity: compound.fidAtPromotion,
  ...
  // EMIT
  //
  // Stamps clauses with step, sequence, and source. Source defaults to
  // PASS_A for templates authored in this module; Pass B detectors set
  // source explicitly when they emit. The stamping preserves any
  // source already set on the clause.
  // -------------------------------------------------------------------
  _emit(clause) {
    const stamped = Object.assign({
      source: CLAUSE_SOURCE.PASS_A
    }, clause, {
      step: Field.step,
...
const ReflexiveSurfaceModule = Object.freeze({
  ...
  PASS_B_TEMPLATES: PASS_B_TEMPLATES,
  templateKeyForCompound: templateKeyForCompound,
  renderPassBTemplate: renderPassBTemplate
});
