...
console.log("Source: constraints-only.js (" + source.length + " bytes)");

const t0 = Date.now();
const sub1 = M1.createStage1Substrate({ id: "s1" });
sub1.ingest(Buffer.from(source));
const vsf1 = sub1.emitVsf();
console.log("Stage 1: " + (Date.now()-t0) + "ms, " + vsf1.split("\n---\n")[1].split("\n").filter(s=>s.length>0).length + " rows");

const t1 = Date.now();
const kp = M2.createStage2Substrate({ id: "kp" });
kp.ingestStage1Vsf(vsf1);
const ks = kp.getState();
console.log("Kind peer: " + (Date.now()-t1) + "ms, " + ks.constraints.length + " constraints, " + ks.subcascades.length + " subs");

const t2 = Date.now();
const tp = Mid.createIdentifierSubstrate({ id: "tp" });
tp.ingestStage1Vsf(vsf1);
const ts = tp.getState();
console.log("Text peer: " + (Date.now()-t2) + "ms, " + ts.constraints.length + " constraints, " + ts.subcascades.length + " subs");

const t3 = Date.now();
const sp = Msa.createStringAnalysisSubstrate({ id: "sp" });
sp.ingestStage1Vsf(vsf1);
const ss = sp.getState();
console.log("String peer: " + (Date.now()-t3) + "ms, " + ss.constraints.length + " constraints, " + ss.subcascades.length + " subs");
...
for (const [k, v] of sorted.slice(0, 20)) {
  console.log("  " + k.padEnd(32) + " uses=" + v.uses.toString().padStart(4) + " kind-bindings=" + v.bindings);
}
